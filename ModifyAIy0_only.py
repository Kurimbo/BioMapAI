...
#'bmi8', 'HDL8', 'LDL8', 'chol8', 'gluc8', 'trig8', 'sbp8', 'dbp8'
# remove third layer
import tensorflow as tf
tf.random.set_seed(1015)
from sklearn.metrics import mean_squared_error
from sklearn.preprocessing import StandardScaler
# Dense has 128 neurons usually
# Dense of 10 equals output (equals # of types)
class ModelConstruct:  #how to differenciate a class from an instance
    #instances are OBJECTS created from classes. ModelConstruct is the  
    #Self allows me to reuse all the parameters/arguments made in parenthesis

    def __init__(self, model_name=None, activation_hidden='relu', # relu = rectified linear activation function, uploads input if threshold is met. Zero otherwise
                 kernel_regularizer=None, 
                 # 0.01 is actually quite high. Rate of steps in the gradient descent. 1e-3 is average
                 optimizer=tf.keras.optimizers.Adam(0.01)): #is a lower value good or bad?
        # this makes the attributes (a,b,c,d) public
...
#---------------------------------------------#
    ...
    def custom_loss_metrics_last_activation(self, y_type): #self is LIKE A POINTER
        if y_type == 'binary': # The outcome here is dead or alive
            loss = 'binary_crossentropy'  #We want to only keep the binary (live/dead)
            metrics = ['accuracy']
...
#---------------------------------------------#
    def y_list_out_metrics(self, y_type, y_dim, previous_layer, hidden_nodes=8): # Self calls the current Class (model construct) in THIS point in time
        out_layer_lists = {}
        custom_loss = {}
        custom_metrics = {}

        #n_y = y_dim.shape[1] if len(y_dim.shape) > 1 else 1
        n_y = y_dim 
        n_y_2 = y_dim #if isinstance(y_dim, int) else len(y_dim)
        assert(n_y == n_y_2)
        
        for i in range(n_y):
            loss, metrics, last_activation = self.custom_loss_metrics_last_activation(y_type[i])
            model_i = tf.keras.layers.Dense(hidden_nodes, activation='relu', kernel_regularizer=self.kernel_regularizer)(previous_layer) # don't touch
            model_i = tf.keras.layers.Dense(y_dim[i], activation=last_activation, name="out{}".format(i))(model_i) # don't touch
            out_layer_lists["out{}".format(i)] = model_i # don't touch
            custom_loss["out{}".format(i)] = loss # don't touch
            custom_metrics["out{}".format(i)] = metrics # don't touch
        return out_layer_lists, custom_loss, custom_metrics
    
#---------------------------------------------#

    def build_model(self, input_dim, y_type, y_dim):  # I would need to edit this part

        #Multi-layer to single-layer

        #Way to fix: I should probably try making a single layer first...
        inputs = tf.keras.layers.Input(shape=(input_dim,)) #layer one
        inputs_norm = tf.keras.layers.BatchNormalization()(inputs) #2

        hidden1 = tf.keras.layers.Dense(64, activation=self.activation_hidden,  #3
        ...
        hidden2 = tf.keras.layers.Dense(32, activation=self.activation_hidden,  #4
        ...
        out_layer_lists, custom_loss, custom_metrics = self.y_list_out_metrics(y_type, y_dim, hidden2)
...
#---------------------------------------------#
    ...
    def y_trans(self, y): # transform is not a matrix/array
        if not isinstance(y, np.ndarray): y = np.asarray(y)
        if y.ndim == 2 and y.shape[1] > 1:
             y = y[:, 0].reshape(-1, 1)  # Only use first column (Y0)
        y_type = self.y_type(y)
        y_dim = y.shape[1]
        return y, y_type, y_dim
#---------------------------------------------#


# Where is this being called (separate model)
class OmicScoreModel: #combined model for each layer (how each layer performs in predicting outcome [not weights, I think])  # look for instance (if present)
 ...
 #---------------------------------------------#
    ...
    def y_preprocess_with_test(self, y_train, y_test):
        ...
        y, y_type, y_dim = self.y_data_processor.y_trans(y_stack)
        y_train_out = y[n_y_train, :]
        y_test_out = y[n_y_test, :]
        return y_train_out, y_test_out, y_type, y_dim
 #---------------------------------------------#
       
    def y_preprocess_no_test(self, y):
        y = self.to_array(y)
        y_out, y_type, y_dim = self.y_data_processor.y_trans(y)
        return y_out, y_type, y_dim
 #---------------------------------------------#
   
    def train_no_test(self, x_train, y_train):
        x_train = self.to_array(x_train)
        y_train = self.to_array(y_train)
        y, y_type,y_dim = self.y_preprocess_no_test(y_train)
        x_train = StandardScaler().fit_transform(x_train)
        model = self.modelconstruct.build_model(input_dim = x_train.shape[1], 
                               y_type= y_type, y_dim=y_dim)
        if not self.batch_size: self.batch_size = x_train.shape[0]
        score_history = model.fit(x_train, y, epochs=self.epochs,
 ...
 #---------------------------------------------#
    ...
    def train_with_test(self, x_train, y_train, x_test, y_test):
        ...
        y_test = self.to_array(y_test)
        y_train, y_test, y_type, y_dim = self.y_preprocess_with_test(y_train, y_test)
        
        x_train = StandardScaler().fit_transform(x_train)
       
        model = self.modelconstruct.build_model(input_dim = x_train.shape[1], 
                               y_type_list=y_type, y_dim_list=y_dim)
        if not self.batch_size: self.batch_size = x_train.shape[0]
        score_history = model.fit(x_train, y_train, epochs=self.epochs,
                                  batch_size=self.batch_size, 
...
#---------------------------------------------#
    ...
    def predict(self, model, x_test):
        ...
        for index, yi in enumerate(predicted_y):
            ...
            else: yi = yi.flatten() # Take as an input, 2D input (one x and one y)
            predicted_y_reshaped[:,index] = yi
        return predicted_y_reshaped
#---------------------------------------------#

    def mse_list(self, y_true_table, y_pred_table): # Diff between y_true and y_pred
        mse_list = []
        y_true = y_true_table
        y_pred = y_pred_table
...
class ScoreLayer:
    ...
    def build_model(self, X, y):
        ...
        model.add(tf.keras.layers.Normalization(axis=-1))  # so if this is normalizing
...
class WeightsAdjust:
    ...
    def adjust_layer_weight(self, model, layer_num, weights):
        ...
        old_weights_flat = old_weights.flatten() #make the old weights... inputtable?
        old_weights_sum = sum(old_weights_flat) # get the sum of the weights
        new_weights = old_weights_flat * weights # get new weights
